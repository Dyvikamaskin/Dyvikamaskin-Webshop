...
> - `data/sku_overlap_report.md` — pairwise overlap matrix across all retailer CSVs + the OEM catalog DB. Regenerate: `python "data/analyze_sku_overlap.py"`.
> - `data/inventory_overlap_report.md` — how `Inventory DM-WN.xlsx` overlaps with every source. Regenerate: `python "data/overlap_inventory_dmwn.py"`.
> - `data/sku_legacy_modern_map.csv` (+ `.json`) — legacy↔modern SAP cross-reference table. Regenerate: `python "data/build_sku_legacy_modern_map.py"`.
...
Scraper: `data/scrape_weidemann_catalog.py`.
...
`PartPriceSnapshot`. CSVs live in `data/`. Counts below are the
...
**Pairwise overlap matrix:** see `data/sku_overlap_report.md` —
...
> `python "data/overlap_inventory_dmwn.py"` →
